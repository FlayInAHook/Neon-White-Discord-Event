                    ...
                    <TableRow key={player.name} className={isHighlighted ? "bg-primary/20 hover:bg-primary/30" : ""}>
                      ...
                      <TableCell className={isHighlighted ? "font-bold" : ""}>
                        {leaderboardType === "solo"
                          ? (activePlayerNamesGlobal.includes(player.name) && <span className="inline-block w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse mr-2" title="Recently active" />)
                          : (player.name.split(" & ").some(n => activePlayerNamesGlobal.includes(n)) && <span className="inline-block w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse mr-2" title="Recently active" />)}
                        {player.name}
                      </TableCell>
                        ...
                        {levelEntries.slice(0, 15).map((entry, index) => (
                          ...
                          >
                            ...
                            <TableCell className={`truncate max-w-[120px] ${entry.name === currentUser ? "font-bold" : ""}`} title={entry.name}>
                              {activePlayersLevel.some(ap => ap.name === entry.name && ap.levelId === level.id) && <span className="inline-block w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse mr-2" title="Recently active" />}
                              {entry.name}
                            </TableCell>
